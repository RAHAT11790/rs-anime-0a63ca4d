    ...
    <nav className="fixed bottom-0 left-0 right-0 h-[65px] z-50 flex items-center justify-around px-2.5 backdrop-blur-[20px] border-t border-primary/15"
      ...
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            {isActive && (
              <span className="absolute top-[-1px] left-1/2 -translate-x-1/2 w-7 h-[3px] rounded-b gradient-primary shadow-[0_2px_10px_hsla(270,70%,55%,0.4)]" />
            )}
            <Icon className={`w-5 h-5 transition-all ${isActive ? "drop-shadow-[0_0_10px_hsla(270,70%,55%,0.5)] scale-110" : ""}`} />
